...
public class ManualGeoGraphicsTest {
	...
	private static final String REFERENCE_PROPERTY = "graphicsviewgl.updateReferences";
	private static final String REFERENCE_RESOURCE_ROOT = "de/sos/gvc/gl/reference";
	...
	private void assertScenario(final Scenario scenario) throws IOException {
		final BufferedImage actual = renderScenario(scenario);
		if (UPDATE_REFERENCES) {
			writeReferenceImage(scenario, actual);
			return;
		}

		final BufferedImage expected = loadReferenceImage(scenario);
		if (expected == null) {
			writeCandidateImage(scenario, actual);
			fail("Missing reference image for " + scenario.name + ". Run with -D" + REFERENCE_PROPERTY + "=true to generate it.");
		}

		ImageCompareUtil.assertEquals(scenario.name, expected, actual, 2, scenario.name + "Failed", TEST_RESULT_DIR.toString());
	}
	...
	private static List<Scenario> scenarios() {
		return Arrays.asList(
				new Scenario("zoom_2_6_center", 2.6, 0, 0),
				new Scenario("zoom_3_0_east_200m", 3.0, 200, 0),
				new Scenario("zoom_3_4_northwest_approx_200m", 3.4, -180, 160),
				new Scenario("zoom_3_8_southeast_approx_200m", 3.8, 220, -190),
				new Scenario("zoom_4_2_south_210m", 4.2, 0, -210));
	}

	private static BufferedImage loadReferenceImage(final Scenario scenario) throws IOException {
		final String resourceName = REFERENCE_RESOURCE_ROOT + "/" + scenario.name + ".png";
		final ClassLoader classLoader = ManualGeoGraphicsTest.class.getClassLoader();
		try (InputStream stream = classLoader.getResourceAsStream(resourceName)) {
			if (stream != null)
				return ImageIO.read(stream);
		}

		final Path file = REFERENCE_SOURCE_DIR.resolve(scenario.name + ".png");
		if (Files.isRegularFile(file))
			return ImageIO.read(file.toFile());
		return null;
	}

	private static void writeReferenceImage(final Scenario scenario, final BufferedImage image) throws IOException {
		Files.createDirectories(REFERENCE_SOURCE_DIR);
		final Path file = REFERENCE_SOURCE_DIR.resolve(scenario.name + ".png");
		ImageIO.write(image, "png", file.toFile());
		System.out.println("Updated reference image: " + file);
	}

	private static void writeCandidateImage(final Scenario scenario, final BufferedImage image) throws IOException {
	...
	private static final class Scenario {
		...
		private final double offsetXMeters;
		private final double offsetYMeters;

		private Scenario(final String name, final double scale, final double offsetXMeters, final double offsetYMeters) {
			this.name = name;
			this.scale = scale;
			this.offsetXMeters = offsetXMeters;
			this.offsetYMeters = offsetYMeters;
		}
	}

	private static final class DeterministicTileImageProvider implements ITileImageProvider {
